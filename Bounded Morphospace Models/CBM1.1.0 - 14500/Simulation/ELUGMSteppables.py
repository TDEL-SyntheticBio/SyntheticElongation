...
BASALYB=40 #baseline motility for L929, due to their extremely motile behaviour, A, B
BASALGR=40 #, A', B'' T0, change GR to 100 - see what happens 
...
MTFORCEMIN=-3*10**(-4.375) #negative mitosis driving force fluctuation, usually only need to change the exponential part
MTFORCEMAX=4*10**(-4.375)  #positive mitosis driving force fluctuation, usually change only the exponential part
MTFORCEMINR=-3*10**(-5.5) #negative mitosis driving force fluctuation, usually only need to change the exponential part
MTFORCEMAXR=4*10**(-5.5)  #positive mitosis driving force fluctuation, usually change only the exponential part
#Signaling Variables
...
THRESHOLDUPYG=10000 #activation threshold to change state, NOTE: line 38 was another set of parameters  
...
THRESHOLDUPBR=14500 #activation threshold to change state, the choice of paramters in this paragraph render it similar to that of the previous paragraph
    ...
    def step(self,mcs):        
        ...
        for cell in self.cellList:
            ...
            if cell.type==4: #this line decides which cells are proliferating; see powerpoint for classification           
                cell.dict["RDM"]+=RNG.uniform(MTFORCEMINR,MTFORCEMAXR) #make cells grow in target radius by this much
                cell.targetSurface=4*math.pi*cell.dict["RDM"]**2 #spherical surface area
                cell.targetVolume=(4/3)*math.pi*cell.dict["RDM"]**3 #spherical volume
                if cell.volume>2*(4/3)*math.pi*RADAVG**3: #divide at two times the mean radius initialized with               
                    cells_to_divide.append(cell)           #add these cells to divide list
